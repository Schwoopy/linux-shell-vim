...
### ☸️ Kubernetes / OpenShift CLI Support

* **kubectl** and **oc** binaries installed into `/usr/local/bin` (or `~/.local/bin` if root write not available)
* Faster:
  * `oc` archive is **stream-extracted** (no intermediate tarball written to disk)
  * `kubectl`/`oc` completion scripts regenerated only when versions change (skipped otherwise)
* Automatically generates **bash-completion** files:
  * `~/.bash_completion.d/kubectl`
  * `~/.bash_completion.d/oc`
* Idempotent: if `kubectl`/`oc` already exist, installer skips replacing the
...
👉 **No EPEL dependency required.**
`epel-release` and `bash-completion-extras` have been removed. Tools not found in base repos are installed **user-scope** (e.g. via `pip` or git clone).
...
- **YAML**: installs `yamllint` and generates a config at `~/.config/yamllint/config`
- **Bash**: installs `shellcheck` and `shfmt` if available in repos; otherwise warns
- **Python**: installs `ruff` and `pylint` via `pip --user` (with version pinning configurable)
...
- Detects OS family automatically
- Installs required packages from base repos (no EPEL)
- Cleans and replaces existing config blocks in `~/.bashrc` idempotently
- Creates backups of modified files (`~/.bashrc.bak.YYYYMMDD_HHMMSS`)
...
- **kubectl** and **oc** binaries installed into `/usr/local/bin` (if missing)
- Automatically generates **bash-completion** files:
  - `~/.bash_completion.d/kubectl`
  - `~/.bash_completion.d/oc`
- Your `.bashrc` block already:
  - Sources system bash-completion if present
  - Auto-loads everything in `~/.bash_completion.d/*`
- Idempotent: if `kubectl`/`oc` already exist, installer skips replacing them
